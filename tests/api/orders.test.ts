...
class TestDatabase {
  ...
  async createTestData(): Promise<{
    ...
    const itemResult = await runQuery(
      ...
      [`ITEM_${Date.now()}`, '测试菜品', '测试菜品描述', categoryId, 28.0, testUser.id],
    );
    const testMenuItem = {
...
describe('订单API', () => {
  ...
  afterAll(async () => {
    await testDb.cleanupTestData(testData.testStore.id, testData.testUser.id, testData.testMember.id);
  ...
  describe('POST /orders', () => {
    it('应该成功创建新订单', async () => {
      ...
      expect(response.data!.subtotal).toBe(56.0);
      expect(response.data!.totalAmount).toBe(56.0);
      ...
      expect(response.data!.items[0]!.unitPrice).toBe(28.0);
      expect(response.data!.items[0]!.totalPrice).toBe(56.0);
  ...
  describe('GET /orders', () => {
    ...
    it('应该支持按门店筛选', async () => {
      const response = await apiRequest<PaginatedOrdersResponse>(`/orders?storeId=${testData.testStore.id}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
  ...
  describe('GET /orders/{id}', () => {
    ...
    it('应该获取订单详情', async () => {
      ...
      expect(response.data!.subtotal).toBe(84.0);
      expect(response.data!.totalAmount).toBe(84.0);
      expect(response.data!.items).toHaveLength(1);
