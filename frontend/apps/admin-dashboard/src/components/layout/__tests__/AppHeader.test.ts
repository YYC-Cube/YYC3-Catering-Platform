...
const commonStubs = {
  ...
  'el-input': {
    template: '<input v-bind:value="value" v-on:input="$emit(\'update:modelValue\', $event.target.value)" />',
    data() {
      return { value: '' };
    },
  },
...
describe('AppHeader', () => {
  ...
  describe('功能测试', () => {
    ...
    it('退出登录功能应该正常工作', async () => {
      ...
      expect(ElMessageBox.confirm).toHaveBeenCalledWith('确定要退出登录吗？', '提示', expect.any(Object));
