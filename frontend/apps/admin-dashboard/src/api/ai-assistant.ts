...
class AIAssistantAPI {
   ...
   * 启用协作增强
   */
  async enableCollaborationEnhancement(
    ...
    try {
      const response: AxiosResponse = await httpClient.post(
        `/ai-assistant/collaboration-enhancements/${enhancementId}/enable`,
        config,
      );
      return response.data;
    } catch (error) {
  ...
  async getCapabilityAnalysis(assistantId: string): Promise<{
    ...
    try {
      const response: AxiosResponse = await httpClient.get(
        `/ai-assistant/assistants/${assistantId}/capability-analysis`,
      );
      return response.data;
    } catch (error) {
