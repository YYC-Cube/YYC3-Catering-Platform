...
export class OrderAPI {
  ...
  static async getOrderByNumber(orderNumber: string): Promise<{ success: boolean; data?: Order; message?: string }> {
    try {
      const response = await httpClient.get<{ success: boolean; data?: Order; message?: string }>(
  ...
  static async createOrder(
    ...
    try {
      const response = await httpClient.post<{ success: boolean; data?: Order; message?: string }>(
        '/orders',
        orderData,
      );
      return response;
  ...
  static async updateOrder(
    ...
    try {
      const response = await httpClient.put<{ success: boolean; data?: Order; message?: string }>(
        `/orders/${id}`,
        orderData,
      );
      return response;
  ...
  static async updateOrderStatus(
    ...
    try {
      const response = await httpClient.patch<{ success: boolean; data?: Order; message?: string }>(
        `/orders/${id}/status`,
        { status },
      );
      return response;
  ...
  static async batchUpdateOrderStatus(
    ...
    try {
      const response = await httpClient.patch<{ success: boolean; message?: string; failed?: number[] }>(
        '/orders/batch/status',
        {
          orderIds,
          status,
...
export const getSalesReport = (params?: { startDate?: string; endDate?: string; groupBy?: 'day' | 'week' | 'month' }) =>
  orderApi.getSalesReport(params);
export const batchUpdateOrderStatus = (orderIds: number[], status: OrderStatus) =>
  orderApi.batchUpdateOrderStatus(orderIds, status);
