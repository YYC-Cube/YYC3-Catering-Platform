...
export class AuthAPI {
  ...
  async verifyToken(token: string): Promise<{ success: boolean; data?: { user: UserInfo }; message?: string }> {
    try {
      const response = await httpClient.get<{ success: boolean; data?: { user: UserInfo }; message?: string }>(
        '/auth/verify',
        {
          headers: {
  ...
  async updateProfile(
    ...
    try {
      const response = await httpClient.put<{ success: boolean; data?: UserInfo; message?: string }>(
        '/auth/profile',
        profileData,
      );
      return response;
  ...
  async checkPermission(permission: string): Promise<boolean> {
    try {
      const response = await httpClient.get<{ success: boolean; data?: { hasPermission: boolean } }>(
