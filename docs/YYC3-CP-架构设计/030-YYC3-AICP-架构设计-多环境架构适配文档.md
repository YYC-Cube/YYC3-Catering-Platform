  ...
  private loadConfig(env: string): any {
    ...
    for (const file of files) {
      if (file.endsWith(".yaml") || file.endsWith(".yml")) {
        const fileConfig = yaml.load(fs.readFileSync(path.join(configPath, file), "utf8"));
...
class CanaryDeploymentService {
  ...
  private async createCanaryDeployment(serviceName: string, image: string, weight: number): Promise<void> {
...
class RollbackService {
  ...
  private shouldRollback(metrics: DeploymentMetrics): boolean {
    return metrics.errorRate > 0.05 || metrics.latency > 2000 || metrics.cpuUsage > 0.9 || metrics.memoryUsage > 0.95;
  }
