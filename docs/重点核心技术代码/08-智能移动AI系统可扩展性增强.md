...
export class ScalabilityEnhancer {
  ...
  async makeScalingDecision(context: ScalingContext): Promise<ScalingDecision> {
    ...
    try {
      ...
      // 5. 识别扩展需求
      const scalingNeeds = await this.identifyScalingNeeds(demandForecast, capacityAssessment);
      ...
      // 10. 做出最终决策
      const decision = await this.makeFinalDecision(scalingPlan, impactAssessment, costBenefit);
  ...
  private horizontalScalingEngine = {
    ...
    loadBasedScaling: async (metric: LoadMetric): Promise<HorizontalScalingResult> => {
      ...
      // 3. 计算扩展数量
      const instancesToAdd = shouldScaleOut ? this.calculateInstancesToAdd(currentLoad) : 0;

      const instancesToRemove = shouldScaleIn ? this.calculateInstancesToRemove(currentLoad) : 0;
      ...
      return {
        ...
        metrics: {
          ...
          targetLoad: shouldScaleOut ? this.config.scaleOutThreshold : this.config.scaleInThreshold,
