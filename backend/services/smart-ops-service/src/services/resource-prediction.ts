...
export class ResourcePredictionService {
   ...
   * @returns 历史数据
   */
  private async getHistoricalData(resourceType: string, window: number): Promise<ResourceUsage[]> {
    // 模拟从数据库获取历史数据
   ...
   * @returns 优化建议
   */
  private generateRecommendation(resourceType: 'cpu' | 'memory' | 'disk' | 'network', predictedUsage: number): string {
