...
class LifecycleRuleEngine {
  ...
  async evaluate(customer: Customer, event: CustomerEvent): Promise<RuleResult[]> {
    const applicableRules = this.rules.filter(rule => rule.fromStage.includes(customer.lifecycleStage) && rule.enabled);
  ...
  private evaluateCondition(condition: RuleCondition, customer: Customer, event: CustomerEvent): boolean {
  ...
  private getFieldValue(field: string, customer: Customer, event: CustomerEvent): any {
    ...
    if (field === 'daysSinceFirstOrder' && customer.registeredAt) {
      return Math.floor((Date.now() - new Date(customer.registeredAt).getTime()) / (1000 * 60 * 60 * 24));
    }
