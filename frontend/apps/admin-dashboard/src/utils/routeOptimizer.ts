 ...
 * 增强路由配置
 */
export function enhanceRoute(route: RouteRecordRaw, group?: RouteGroup, priority?: RoutePriority): RouteRecordRaw {
 ...
 * 按分组过滤路由
 */
export function filterRoutesByGroup(routes: RouteRecordRaw[], group: RouteGroup): RouteRecordRaw[] {
 ...
 * 按优先级过滤路由
 */
export function filterRoutesByPriority(routes: RouteRecordRaw[], priority: RoutePriority): RouteRecordRaw[] {
  return routes.filter(route => {
    const meta = route.meta as EnhancedRouteMeta;
...
export class RoutePreloader {
  ...
  startIdleListener() {
    ...
    if ('requestIdleCallback' in window) {
      this.idleCallbackId = window.requestIdleCallback(() => this.preloadHighPriorityRoutes(), {
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        const result = await Promise.race([
          importFn(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Route load timeout')), timeout)),
        ]);
        return result;
      } catch (error) {
