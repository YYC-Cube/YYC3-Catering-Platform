...
export class NLPService {
  ...
  private initializeIntentPatterns(): Map<string, RegExp[]> {
    ...
    // 询问价格意图
    patterns.set('ask_price', [/(.+)多少钱|(.+)的价格/i, /(价格|多少钱)(是多少|怎么样)(.+)吗?/i]);

    // 取消订单意图
    patterns.set('cancel_order', [/(取消|撤销)(订单|我的订单)/i, /(我想|帮我)(取消|撤销)(订单|我的订单)/i]);

    // 投诉意图
    patterns.set('complain', [/(投诉|抱怨|不满|不好|太差)(.+)/i, /(我要|我想)(投诉|抱怨)(.+)/i]);

    // 表扬意图
    patterns.set('praise', [/(好吃|不错|很好|太棒了|优秀|表扬)(.+)/i, /(赞|好)(.+)/i]);
    ...
    // 询问位置意图
    patterns.set('ask_location', [/(你们的|餐厅的)(位置|地址)(在哪里|是什么)/i, /(地址|位置)(是多少|怎么样|在哪里)/i]);
  ...
  private initializeEntityPatterns(): Map<string, RegExp[]> {
    ...
    // 食物类别实体
    patterns.set('food_category', [/(招牌菜|特色菜|凉菜|热菜|主食|汤品|甜品|饮料|酒水|小吃)/gi]);
    ...
    // 过敏原实体
    patterns.set('allergen', [/(坚果|花生|海鲜|虾|蟹|贝类|牛奶|鸡蛋|小麦|大豆|麸质|芝麻|鱼|蜂蜜)/gi]);

    // 饮食限制实体
    patterns.set('dietary_restriction', [/(素食|纯素|清真|无麸质|低糖|低脂|低卡路里|减肥|糖尿病|高血压|高血脂)/gi]);

    // 价格实体
    patterns.set('price', [/(\d+)(\.\d+)?(元|块|角|分)/gi, /¥(\d+)(\.\d+)?/gi, /\$(\d+)(\.\d+)?/gi]);

    // 位置实体
    patterns.set('location', [/(地址|位置|在哪里|怎么走|路线)(.+)/gi]);
        ...
        try {
          ...
          while ((match = pattern.exec(text)) !== null) {
            ...
            const alreadyExists = entities.some(
              entity => entity.start === match.index && entity.end === match.index + match[0].length,
            );
  ...
  async processText(text: string): Promise<NLPResult> {
    ...
    const entityConfidence =
      entities.length > 0 ? entities.reduce((sum, entity) => sum + entity.confidence, 0) / entities.length : 0.5;
