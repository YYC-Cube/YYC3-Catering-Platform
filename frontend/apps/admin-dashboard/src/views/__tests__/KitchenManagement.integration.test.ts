  ...
  describe('数据分析集成', () => {
    it('应该成功加载分析数据', async () => {
      const mockAnalytics = {
        ...
        stationDistribution: [{ station: 'hot_dish', count: 10, percentage: 50 }],
        staffEfficiency: [
          { employeeId: 'emp-1', employeeName: '张三', tasksCompleted: 50, averageTime: 12, efficiency: 92.5 },
        ],
        peakHours: [{ hour: 12, taskCount: 20, orderCount: 15 }],
        equipmentUtilization: [{ equipmentId: 'eq-1', equipmentName: '蒸柜', usageHours: 8, utilizationRate: 80 }],
        inventoryTrends: [{ date: '2025-01-20', itemCount: 100, value: 50000 }],
      };
