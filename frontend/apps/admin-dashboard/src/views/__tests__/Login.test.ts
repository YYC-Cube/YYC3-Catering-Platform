...
const commonStubs = {
  ...
  'el-input': {
    template:
      '<input v-bind:value="value" v-on:input="$emit(\'update:modelValue\', $event.target.value)" v-on="$attrs" />',
    data() {
      return { value: '' };
    },
  },
  ...
  'el-checkbox': {
    template:
      '<input type="checkbox" v-bind:checked="modelValue" v-on:change="$emit(\'update:modelValue\', $event.target.checked)" />',
    props: ['modelValue'],
  },
