...
export class MemberAPI {
  ...
  async getMemberByPhone(phone: string): Promise<{ success: boolean; data?: Member; message?: string }> {
    try {
      const response = await httpClient.get<{ success: boolean; data?: Member; message?: string }>(
        `/members/phone/${phone}`,
      );
      return response;
    } catch (error) {
  ...
  async createMember(memberData: CreateMemberRequest): Promise<{ success: boolean; data?: Member; message?: string }> {
    try {
      const response = await httpClient.post<{ success: boolean; data?: Member; message?: string }>(
        '/members',
        memberData,
      );
      return response;
  ...
  async updateMember(
    ...
    try {
      const response = await httpClient.put<{ success: boolean; data?: Member; message?: string }>(
        `/members/${id}`,
        memberData,
      );
      return response;
  ...
  async upgradeMemberLevel(memberId: number, newLevel: MemberLevel): Promise<{ success: boolean; message?: string }> {
    try {
      const response = await httpClient.patch<{ success: boolean; message?: string }>(`/members/${memberId}/level`, {
        level: newLevel,
      });
      return response;
    } catch (error) {
...
export const getPointsHistory = (memberId: number, page?: number, limit?: number) =>
  memberApi.getPointsHistory(memberId, page, limit);
export const getMemberStats = (params?: { startDate?: string; endDate?: string }) => memberApi.getMemberStats(params);
export const upgradeMemberLevel = (memberId: number, newLevel: MemberLevel) =>
  memberApi.upgradeMemberLevel(memberId, newLevel);
export const batchImportMembers = (members: CreateMemberRequest[]) => memberApi.batchImportMembers(members);
export const exportMembers = (format?: 'csv' | 'excel', filters?: MemberQueryParams) =>
  memberApi.exportMembers(format, filters);
export const sendNotification = (memberIds: number[], message: string, type: 'sms' | 'email' | 'system') =>
  memberApi.sendNotification(memberIds, message, type);
