...
class OrderAPI {
  ...
  async updateOrderFlow(
    id: number,
    status: OrderStatus,
  ): Promise<{ success: boolean; data?: OrderFlow; message?: string }> {
    try {
  ...
  async updateKitchenDisplay(
    id: number,
    display: Partial<KitchenDisplay>,
   ...
   * 获取订单时间线
   */
  async getOrderTimeline(id: number): Promise<{
    success: boolean;
    data?: Array<{
      timestamp: string;
      status: OrderStatus;
  ...
  async recalculateOrder(
    id: number,
  ): Promise<{ success: boolean; data?: { totalAmount: number; finalAmount: number }; message?: string }> {
    try {
