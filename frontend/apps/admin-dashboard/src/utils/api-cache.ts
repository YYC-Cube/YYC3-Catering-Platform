...
export class CachePrefetcher {
  ...
  // 添加预取任务
  addPrefetch(request: RequestInfo, fetcher: () => Promise<any>, priority: number = 0): void {
...
export class CacheInvalidator {
  ...
  // 添加失效规则
  addRule(pattern: RegExp | string, action: 'delete' | 'refresh', refreshFetcher?: () => Promise<any>): void {
    this.invalidationRules.push({ pattern, action, refreshFetcher });
  }

  // 执行失效
  async invalidate(cacheManager: APICacheManager, trigger: string): Promise<void> {
...
export function createCacheMiddleware(cacheManager: APICacheManager) {
  return async (request: RequestInfo, next: () => Promise<any>): Promise<any> => {
    // 尝试从缓存获取
    const cached = cacheManager.get(request);
    if (cached) {
      return cached.data;
