...
export const Tree = defineComponent({
  ...
  setup(props, { emit, slots }) {
      ...
      return (
        <div key={key} class="tree-node">
          ...
          >
            ...
            {props.showIcon && node.icon && <node.icon size={16} class="text-neutral-600" />}
            <span class="flex-1 truncate">{node.title}</span>
            {node.extra && <div class="flex-shrink-0">{node.extra}</div>}
          </div>
          {expanded && node.children && node.children.length > 0 && (
            <div class="tree-children">{node.children.map(child => renderTreeNode(child, level + 1))}</div>
          )}
        </div>
      );
    };
    ...
    return () => (
      <div
        class={cn('w-full', props.showLine && 'tree-show-line')}
        style={props.height ? { height: `${props.height}px`, overflow: 'auto' } : {}}
      >
        {props.treeData.length === 0 ? (
          <div class="flex items-center justify-center py-8 text-neutral-500">暂无数据</div>
        ) : (
