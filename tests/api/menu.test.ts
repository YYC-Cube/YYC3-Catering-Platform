...
class TestDatabase {
  ...
  async createTestData(): Promise<{
    ...
    // 登录获取token
    const loginResponse = await apiRequest<{ success: boolean; data?: { token: string; user?: { id: string } } }>(
      '/auth/login',
      {
        method: 'POST',
...
describe('菜单API', () => {
  ...
  afterAll(async () => {
    try {
      if (testData?.testUser?.id) {
        await testDb.cleanupTestData(testData.testUser.id, testData.testCategory?.id);
      }
  ...
  describe('POST /menu/items', () => {
    it('应该成功创建新菜品', async () => {
      ...
      expect(response.data?.price).toBe(28.0);
      expect(response.data?.originalPrice).toBe(32.0);
  ...
  describe('GET /menu/items', () => {
    ...
    it('应该支持按价格范围筛选', async () => {
      ...
      if (response.data?.items?.length > 0) {
        response.data.items.forEach(item => {
          expect(item?.price).toBeGreaterThanOrEqual(20.0);
          expect(item?.price).toBeLessThanOrEqual(30.0);
        });
      }
    });
  ...
  describe('GET /menu/items/:id', () => {
    ...
    it('应该获取菜品详情', async () => {
      ...
      expect(response.data?.price).toBe(38.0);
  ...
  describe('PUT /menu/items/:id', () => {
    ...
    it('应该成功更新菜品', async () => {
      ...
      expect(response.data?.price).toBe(22.0);
  ...
  describe('GET /menu/search', () => {
    ...
    it('应该支持价格范围筛选的搜索', async () => {
      ...
      if (response.data?.items?.length > 0) {
        response.data.items.forEach(item => {
          expect(item?.price).toBeGreaterThanOrEqual(30.0);
          expect(item?.price).toBeLessThanOrEqual(40.0);
        });
      }
    });
