...
| 命名对象 | 规范                                           | 示例                                          |
| -------- | ---------------------------------------------- | --------------------------------------------- |
| 数据库名 | 服务名称\_环境，使用小写字母和下划线           | yyc3_user_service_dev, yyc3_user_service_prod |
| 表名     | 使用小写字母和下划线，采用复数形式             | users, menus, orders                          |
| 字段名   | 使用小写字母和下划线，避免使用保留字           | user_id, menu_name, order_time                |
| 索引名   | 表名\_字段名\_index，使用小写字母和下划线      | users_email_index, orders_user_id_index       |
| 外键名   | 表名*引用表名*字段名\_fk，使用小写字母和下划线 | orders_users_user_id_fk                       |
...
3. **存储过程命名**：如果必须使用存储过程，使用sp\_前缀，后跟存储过程名称
4. **触发器命名**：如果必须使用触发器，使用tr\_前缀，后跟触发器名称
...
const startApp = async () => {
  try {
    ...
    const server = app.listen(config.server.port, config.server.host, () => {
      logger.info(`🚀 ${config.service.name} v${config.service.version} 已启动`, {
...
export class UserService {
  ...
  public async getUsers(options: PaginationOptions): Promise<PaginationResult<UserDto>> {
    ...
    const [users, total] = await Promise.all([this.userRepository.findAll(limit, offset), this.userRepository.count()]);
