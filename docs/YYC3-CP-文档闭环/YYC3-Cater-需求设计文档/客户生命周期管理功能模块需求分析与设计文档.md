...
class RFMModel {
  calculateRFMScore(customer: Customer): RFMScore {
    ...
    const totalScore = recencyScore * 0.3 + frequencyScore * 0.4 + monetaryScore * 0.3;
...
class CareReminderEngine {
  ...
  private async evaluateTrigger(rule: CareReminderRule, customer: Customer, event: CustomerEvent): Promise<boolean> {
  ...
  private async sendCareReminder(customer: Customer, rule: CareReminderRule): Promise<void> {
