...
export function addCustomerStatus(
  customerId: string,
  data: AddStatusRequest
): Promise<{
  success: boolean;
  message: string;
}>;
...
```typescript
export function updateCustomerSegment(
  segmentId: string,
...
function calculateKeyMetrics(customers: Customer[], timeRange: TimeRange): KeyMetrics {
  ...
  const avgCustomerValue =
    totalCustomers > 0 ? filteredCustomers.reduce((sum, c) => sum + (c.totalSpent || 0), 0) / totalCustomers : 0;
...
function calculateMetricValue(customers: Customer[], metric: string): number {
  switch (metric) {
    ...
    case "avgCustomerValue":
      return customers.length > 0 ? customers.reduce((sum, c) => sum + (c.totalSpent || 0), 0) / customers.length : 0;
    default:
      return 0;
  }
}
...
function calculateCohortAnalysis(customers: Customer[], cohortPeriod: "month" | "quarter"): CohortAnalysis {
  ...
  const cohortArray = Array.from(cohorts.values()).sort(
