...
class UsageBillingAPI {
   ...
   * 获取使用量定价配置列表
   */
  async getPricingConfigs(params?: { type?: BillingType; active?: boolean; limit?: number; offset?: number }): Promise<{
    success: boolean;
    data?: {
   ...
   * 获取使用量预算列表
   */
  async getBudgets(params?: { metricType?: string; active?: boolean; limit?: number; offset?: number }): Promise<{
    success: boolean;
    data?: {
      budgets: UsageBudget[];
      total: number;
