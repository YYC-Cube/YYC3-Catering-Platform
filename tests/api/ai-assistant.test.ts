...
// 测试工具函数
async function apiRequest<T = any>(endpoint: string, options: RequestInit = {}, token?: string): Promise<T> {
...
class TestDatabase {
  ...
  async createUser(userData: typeof testUser): Promise<number> {
    ...
    const result = stmt.run(userData.username, userData.email, passwordHash, 'salt', userData.fullName, userData.role);
...
describe('AI助手API', () => {
  ...
  describe('POST /ai/message', () => {
    it('应该成功处理文本消息', async () => {
      ...
      const response = await apiRequest<AIAssistantResponse>(
        '/ai/message',
        {
          method: 'POST',
    ...
    it('应该验证必填字段', async () => {
      ...
      try {
        await apiRequest<AIAssistantResponse>(
          '/ai/message',
          {
            method: 'POST',
  ...
  describe('会话管理', () => {
    it('应该获取会话历史记录', async () => {
      ...
      // 获取历史记录
      const response = await apiRequest<AIAssistantResponse>(
        `/ai/conversation/${testSessionId}/history`,
        {
          method: 'GET',
        },
        authToken,
    ...
    it('应该清除会话历史记录', async () => {
      ...
      // 验证历史记录已清除
      const historyResponse = await apiRequest<AIAssistantResponse>(
        `/ai/conversation/${testSessionId}/history`,
        {
          method: 'GET',
        },
    ...
    it('应该获取会话统计信息', async () => {
      ...
      // 获取会话统计
      const response = await apiRequest<AIAssistantResponse>(
        `/ai/conversation/${testSessionId}/stats`,
        {
          method: 'GET',
        },
        authToken,
