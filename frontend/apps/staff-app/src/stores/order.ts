...
export const useOrderStore = defineStore('order', () => {
  ...
  const orderStats = computed(() => ({
    ...
    totalRevenue: orders.value.filter(o => o.status === 'completed').reduce((sum, o) => sum + o.totalAmount, 0),
  }));
  ...
  const filteredOrders = computed(() => {
    ...
    if (filters.value.tableId) {
      filtered = filtered.filter(order => order.tableId === filters.value.tableId);
    }

    if (filters.value.customerId) {
      filtered = filtered.filter(order => order.customerId === filters.value.customerId);
    }
  ...
  const filteredKitchenOrders = computed(() => {
    ...
    if (kitchenFilters.value.status?.length) {
      filtered = filtered.filter(order => kitchenFilters.value.status!.includes(order.status));
    }

    if (kitchenFilters.value.chefId) {
      filtered = filtered.filter(order => order.assignedChefId === kitchenFilters.value.chefId);
    }

    if (kitchenFilters.value.priority) {
      filtered = filtered.filter(order => order.priority >= kitchenFilters.value.priority!);
    }
  ...
  const updateOrderStatus = async (orderId: string, status: OrderStatus, notes?: string): Promise<Order> => {
    return updateOrder(orderId, { status, notes });
  };

  const updateOrderItem = async (orderId: string, itemId: string, updates: Partial<OrderItem>): Promise<Order> => {
    try {
  ...
  const refreshOrders = async () => {
    await Promise.all([loadOrders(), loadPendingOrders(), loadKitchenOrders()]);
  };

  // 私有方法
  const updateDerivedOrders = () => {
    // 更新待处理订单
    pendingOrders.value = orders.value.filter(order => ['pending', 'confirmed'].includes(order.status));
  ...
  const calculateAveragePrepTime = (): number => {
    const completedOrders = kitchenOrders.value.filter(o => o.status === 'completed' && o.preparationTime);

    if (completedOrders.length === 0) return 0;

    const totalTime = completedOrders.reduce((sum, order) => sum + order.preparationTime!, 0);
