...
class CareReminderEngine {
  ...
  private initializeDefaultRules(): void {
    this.rules = [
      ...
      },
      {
        ...
        contentTemplate:
          '亲爱的{name}，最近很少见到您，是有什么不满意的地方吗？我们为您准备了专属优惠券，期待您的光临！',
        channels: ['sms', 'push'],
        priority: 4,
        enabled: true,
        createdAt: new Date(),
  ...
  private async evaluateTrigger(rule: CareReminderRule, customer: Customer, event: any): Promise<boolean> {
  ...
  private async createReminder(customer: Customer, rule: CareReminderRule): Promise<CareReminder> {
  ...
  private generateContent(template: string, customer: Customer, rule: CareReminderRule): string {
  ...
  getEffectivenessStatistics(): any {
    ...
    const avgEffectiveness =
      sentReminders.reduce((sum, r) => sum + (r.effectivenessScore || 0), 0) / sentReminders.length;
