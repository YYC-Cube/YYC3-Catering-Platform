...
vi.mock('@/api/chain', () => ({
  ChainAPI: vi.fn().mockImplementation(() => ({
    ...
    getEmployeePerformanceOverview: vi.fn(() =>
      Promise.resolve({
        success: true,
        data: [
          {
...
describe('ChainManagement组件', () => {
  ...
  describe('库存管理', () => {
    ...
    it('应该正确显示商品名称', () => {
      ...
      const hasProductName = productNameCells.some(
        cell => cell.text().includes('牛肉') || cell.text().includes('鸡肉'),
      );
