  ...
  setup(props, { emit }) {
    ...
    const filteredOptions = computed(() => {
      ...
      return props.options.filter(option => {
        if (option.children) {
          return option.children.some(child => child.label.toLowerCase().includes(query));
        }
      ...
      <div ref={selectRef} class="relative w-full">
        <div
          class={cn(
            ...
            visible.value
              ? 'border-primary-500 ring-primary-500/50'
            ...
            {props.multiple && Array.isArray(props.modelValue) && props.modelValue.length > 0 ? (
              <div class="flex items-center gap-1 flex-wrap">
                ...
                {props.maxTagCount !== undefined && selectedOptions.value.length > props.maxTagCount && (
                  <span class="text-neutral-500 text-xs">+{selectedOptions.value.length - props.maxTagCount}</span>
                )}
              </div>
            ) : (
              <span class={cn('truncate', !props.modelValue && 'text-neutral-400')}>{displayValue.value}</span>
            )}
          </div>
            ...
            {props.loading ? (
              ...
              <div class="py-1">
                {Object.entries(getOptionGroups()).map(([groupName, options]) => (
                  <div key={groupName}>
                    <div class="px-3 py-2 text-xs font-semibold text-neutral-500 bg-neutral-50">{groupName}</div>
                ...
                {filteredOptions.value.length === 0 && (
                  <div class="px-3 py-8 text-center text-neutral-500 text-sm">无数据</div>
                )}
              </div>
            )}
          </div>
