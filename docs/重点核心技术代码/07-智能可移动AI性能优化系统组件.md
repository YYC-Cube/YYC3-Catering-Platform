...
export class PerformanceOptimizer {
  ...
  private optimizationEngine = {
    ...
    selectOptimalStrategy: async (strategies: OptimizationStrategy[]): Promise<SelectedStrategy> => {
      // 1. 多维度评估
      const evaluations = await Promise.all(strategies.map(strategy => this.evaluateStrategy(strategy)));
  ...
  private async continuousOptimizationPipeline(): Promise<void> {
    while (this.isRunning) {
      try {
        ...
        // 4. 决策阶段
        const decision = await this.optimizationEngine.selectOptimalStrategy(diagnosisResult.recommendedStrategies);
