...
class UserServiceTest extends JestTestBase {
  ...
  static testCreateUser() {
    ...
    it("当邮箱为空时应该抛出错误", async () => {
      ...
      await expect(this.userService.createUser(userData)).rejects.toThrow("邮箱不能为空");
    });
  }
}
```
  ...
  describe("用户API", () => {
    ...
    describe("GET /api/users/:id", () => {
      ...
      it("当用户不存在时应该返回404错误", async () => {
        await request(app).get("/api/users/nonexistent-id").set("Authorization", `Bearer ${authToken}`).expect(404);
      });
    });
    ...
    describe("DELETE /api/users/:id", () => {
      it("应该成功删除用户", async () => {
        await request(app).delete(`/api/users/${userId}`).set("Authorization", `Bearer ${authToken}`).expect(204);

        // 验证用户已被删除
        await request(app).get(`/api/users/${userId}`).set("Authorization", `Bearer ${authToken}`).expect(404);
      });
    });
  });
});
...
describe("数据库集成测试", () => {
  ...
  describe("用户表操作", () => {
    it("应该成功插入用户", async () => {
      ...
      const result = await db.query("INSERT INTO users (email, name, password) VALUES ($1, $2, $3) RETURNING *", [
        userData.email,
        userData.name,
        userData.password,
      ]);
    ...
    it("应该成功查询用户", async () => {
      // 先插入用户
      await db.query("INSERT INTO users (email, name, password) VALUES ($1, $2, $3)", [
        "test@example.com",
        "Test User",
        "hashed_password",
      ]);
    ...
    it("应该成功更新用户", async () => {
      ...
      // 更新用户
      await db.query("UPDATE users SET name = $1 WHERE id = $2", ["Updated User", userId]);
  ...
  describe("事务操作", () => {
    it("应该成功执行事务", async () => {
      await db.transaction(async client => {
        ...
        // 插入订单
        await client.query("INSERT INTO orders (user_id, total_amount, status) VALUES ($1, $2, $3)", [
          userId,
          100.0,
          "pending",
        ]);
      ...
      // 验证数据已插入
      const userResult = await db.query("SELECT * FROM users WHERE email = $1", ["test@example.com"]);

      const orderResult = await db.query("SELECT * FROM orders WHERE user_id = $1", [userResult.rows[0].id]);
      ...
      try {
        await db.transaction(async client => {
          ...
          await client.query(
            ...
            [userId, -100.0, "pending"] // 负数金额会触发约束
          );
        });
      } catch (error) {
        // 预期会抛出错误
