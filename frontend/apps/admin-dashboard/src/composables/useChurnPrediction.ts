...
class ChurnPredictionModel {
  ...
  private calculateMonthlyFrequency(customer: Customer, days: number): number {
    ...
    const recentOrders =
      customer.recentOrders?.filter(order => {
  ...
  getChurnStatistics(predictions: ChurnPrediction[]): any {
    ...
    const avgChurnProbability =
      predictions.length > 0 ? predictions.reduce((sum, p) => sum + p.churnProbability, 0) / predictions.length : 0;
  ...
  getTrendAnalysis(predictions: ChurnPrediction[]): any {
    ...
    const weekTrend =
      previousWeekHighRisk > 0 ? ((currentWeekHighRisk - previousWeekHighRisk) / previousWeekHighRisk) * 100 : 0;
...
export function useChurnPrediction() {
  ...
  const getUrgency = (prediction: ChurnPrediction): string => {
    const daysUntilChurn = Math.floor((prediction.predictedChurnDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
