  ...
  public validateQuery(schema: any) {
    return async (request: Request): Promise<ValidationResult> => {
      try {
        ...
        if (!result.success) {
          const errors = result.error.errors.map(
            (err: any) => `query.${err.path.join('.')}: ${err.message}`
          );
  ...
  public validateParams(schema: any) {
    return async (request: Request, params?: Record<string, string>): Promise<ValidationResult> => {
      try {
        ...
        if (!result.success) {
          const errors = result.error.errors.map(
            (err: any) => `params.${err.path.join('.')}: ${err.message}`
          );
  ...
  public validateHeaders(schema: any) {
    return async (request: Request): Promise<ValidationResult> => {
      try {
        ...
        if (!result.success) {
          const errors = result.error.errors.map(
            (err: any) => `headers.${err.path.join('.')}: ${err.message}`
          );
...
export const validateFile = (file: File, options?: any) =>
  validationMiddleware.validateFile(file, options);
