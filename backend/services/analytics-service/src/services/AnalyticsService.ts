...
export class AnalyticsService {
  ...
  async getSalesStats(startDate: Date, endDate: Date): Promise<Record<string, any>> {
    try {
      ...
      // 按支付方式分组
      const paymentMethodStats = orders.reduce(
        (acc, order) => {
   ...
   * @returns Promise<Record<string, any>>
   */
  async getUserActivityStats(userId: number, startDate: Date, endDate: Date): Promise<Record<string, any>> {
    try {
      ...
      // 按事件类型分组
      const eventTypeStats = userActivities.reduce(
        (acc, activity) => {
   ...
   * @returns Promise<Record<string, any>[]>
   */
  async getPopularProducts(startDate: Date, endDate: Date, limit: number = 10): Promise<Record<string, any>[]> {
    try {
      ...
      // 计算产品统计
      const productStats = productData.reduce(
        (acc, data) => {
